...
def cardiac():
    ...
    if len(probability_new) == 9:
        probability_final = float(
            probability_new[6:]
        )  # Remove the first 11 and last 1 characters
    else:
        probability_final = float(
            probability_new[13:][:-1]
        )  # Remove the first 11 and last 1 characters
...
def diabetes():
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        converted_data.drop("class_Positive", axis=1),
        converted_data["class_Positive"],
        test_size=0.3,
        random_state=0,
    )
    ...
    process_prediction = RF_classifier.predict(
        sc.transform(
            np.array(
                [
                    [
